...
database.ref('/incubadora').update({salida:0});
...
var starCountRef = firebase.database().ref('incubadora/' + '/humedad');
...
var starCountRef = firebase.database().ref('incubadora/' + '/temperaturac');
starCountRef.on('value', function(snapshot) {
    var temperaturac=snapshot.val();
    
    var start = firebase.database().ref('incubadora/' + '/temperaturaf');
...
var starCountRef = firebase.database().ref('incubadora/' + '/indicec');
starCountRef.on('value', function(snapshot) {
    var temperaturac=snapshot.val();
    
    var start = firebase.database().ref('incubadora/' + '/indicef');
    start.on('value', function(snapshot) {
        var temperaturaf=snapshot.val();
...
function lampara(){
if(lamparain==false){
    database.ref('/incubadora').update({
        salida:1});
    lamparain=true;
...
/*var starCountRef = firebase.database().ref('incubadora/' + '/color');
starCountRef.on('value', function(snapshot) {
